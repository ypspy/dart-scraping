...
#### 입수파일명
파일명은 입수 자료와 회사의 정보로 구성되어 있다. 아래에 예시 파일과 파일명에 포함된 정보를 설명하는 표를 작성하였다. 압축파일에 소용량 파일이 다수 포함되어 있다. 반디 소프트(https://www.bandisoft.com/)의 반디집 사용을 추천한다. 
...
이러한 어려움을 해결하기 위해, AWS의 EC2를 여러개 생성하여 공시자료를 연도별로 동시에(병렬로) 입수하였다. 그 결과 2019년 공시까지 약 3주 정도의 시간이 경과되었다. 사용하는 EC2의 수에 따라 더 단축가능하다.

## 입수 데이터

### (A) Disclosure-to-HTML (Python 3)

2020년 Scraping한 HTML 파일. 등록일 기준으로 요약된 자료이며 후속 가공이 필요하다. 
...
<표 2> DART에서 입수한 HTML 파일(CSS/Image없음)
