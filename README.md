...
- DART 공시원문 활용 : DART에 공시되는 공시보고서의 원문을 XML형식으로 다운로드받아 원하는 자료를 자유롭게 추출하여 사용
- 주요공시 및 재무정보 제공 : 사업보고서 주요항목 및 주요재무계정, 지분보고서 종합정보를 데이터 형식으로 바로 활용   
...
이하 소개하는 코드는 여러 상황에 오픈API를 보완한다. 

> 첫번째, 입수 코드는 입수단위가 목적에 맞게 분리되어 있다. FSS 오픈 API의 원문정보 자료의 경우 사업보고서와 감사보고서가 한덩어리로 입수되어 후가공에서 추가 분리가 필요하여, 이러한 경우 text를 분리하는 룰이 필요하며, 예외 처리 등의 어려움이 있다. 
...
한편, HTML로 작성된 다트 공시파일은 CSS로 입혀진 공통 서식과 <img> 태그로 붙혀지는 이미지 파일이 포함되어있다. 입수 코드는 CSS와 이미지는 가져오지 않고, Bare HTML만 입수한다. 본 입수코드와 후속가공의 목적이 공시자료에 포함된 Text자료의 입수에 있기 때문에 이미지 정보는 입수 불필요하다. 특히, 감사보고서와 첨부된 FS에는 이미지가 붙어있는 사례를 알지 못하여 제거하였다. (스타일은 말할 필요도 없다.)


#### 사용방법
Anaconda를 설치하면 따라오는 통합개발환경인 Spyder에 코드를 붙여서 쓴다. Anaconda 설치시 BeautifulSoup, requests 같은 라이브러리가 따라오기 때문에 사용이 간편하다. 이는 이하, Python을 사용한 모든 코드에 동일하다.

scraper 코드의 경우 아래 부분을 필요에 맞게 조정하여 입수한다. 필요하다면 코드를 수정하여 코드 실행 후 콘솔에서 필요한 변수를 지정할 수 있다. (설명을 위해 Comment 삭제한다. 이하 python 코드에 대한 설명은 모두 같다.)
...
이러한 어려움을 해결하기 위해, AWS의 EC2를 여러개 생성하여 공시자료를 접수연도별로 동시에(병렬로) 입수하였다. 그 결과 2019년 공시까지 약 3주 정도의 시간이 경과되었다. 이는 사용하는 EC2의 수에 따라 더 단축가능하다.

AWS에서 입수한 파일은 압축 후 로컬로 옮겨 가공한다. 

### (C) Local에서 Path 생성

#### 코드: poolingFiles (Python 3)
...
그러나 최종 가공 데이터는 사업연도로 정리할 필요가 있다. 접수일 자체는 보고서의 재무적 특징을 담고 있지 않아 Data 분석의 기준이 되지 못하기 때문이다. 따라서 코드는 보고서, 접수연도 단위로 입수된 보고서 중 필요한 서류(예: 외부감사 실시내용, 감사보고서 본문, 사업보고서 표지 등)를 분리한 경로리스트를 만들고, 이를 연도별로 분할하여 후속 분석에 사용한다. 프로세스의 주 목적에 더하여 

#### 사용방법
poolingFiles 코드의 경우 아래 부분을 필요에 맞게 조정하여 입수한다. 필요하다면 코드를 수정하여 코드 실행 후 콘솔에서 필요한 변수를 지정할 수 있다. 
...
<표 2> pooling Files 사용법 설명
변수(예시) | 설명
--- | ---
os.chdir("F:\workingDirectory\\") | 입수 파일(서브폴더)가 모여있는 작업 폴더 선택
for path in [".\A001_2017\\", ".\A001_2018\\", ".\A001_2019\\", ".\A001_2020\\"]: | 폴더가 분리되어 있다면, 경로 추출 목표인 폴더의 상대 경로 리스트에 반영
path = path + "\*외부감사실시내용\*.\*" | 필요한 키워드 입력 예: 표지(사업보고서_사업보고서), 감사보고서 표지(감사보고서_감사보고서), 감사보고서 외부감사실시내용("외부감사실시내용")
