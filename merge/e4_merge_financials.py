...
# 총자산(total_asset) 정보
df2 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data07.output.csv'), sep="\t")
df2 = df2.loc[:, ~df2.columns.str.contains('^Unnamed')]  # 제거

# 자본총계 정보
df3 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data09.output.csv'), sep="\t")
df3 = df3.loc[:, ~df3.columns.str.contains('^Unnamed')]  # 제거

# 당기순손실 정보
df4 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data14.output.csv'), sep="\t")
df4 = df4.loc[:, ~df4.columns.str.contains('^Unnamed')]  # 제거

# 재고자산
df5 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data15.output.csv'), sep="\t")
df5 = df5.loc[:, ~df5.columns.str.contains('^Unnamed')]  # 제거

# 매출채권
df6 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data16.output.csv'), sep="\t")
df6 = df6.loc[:, ~df6.columns.str.contains('^Unnamed')]  # 제거

# 장기매출채권
df7 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data11.output.csv'), sep="\t")
df7 = df7.loc[:, ~df7.columns.str.contains('^Unnamed')]  # 제거

# 계약자산
df8 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data17.output.csv'), sep="\t")
df8 = df8.loc[:, ~df8.columns.str.contains('^Unnamed')]  # 제거

# 미청구공사
df9 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data18.output.csv'), sep="\t")
df9 = df9.loc[:, ~df9.columns.str.contains('^Unnamed')]  # 제거

# 감사보고서 본문의 의견
df10 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data08.output.csv'), sep="\t")
...
df.to_csv(os.path.join(OUTPUT_DIR, "wp01.data.financials.csv"), sep="\t")
