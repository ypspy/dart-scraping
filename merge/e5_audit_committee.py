...
df2 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data19.output.csv'), sep="\t")
df2 = df2.loc[:, ~df2.columns.str.contains('^Unnamed')]  # 제거
df2["subKey"] = df2["5"] + df2["10"]

# 사업보고서 첨부 정관에서 감사/감사위원회 정보 추출
df3 = pd.read_csv(os.path.join(OUTPUT_DIR, 'wp01.data20.output.csv'), sep="\t")
...
df.to_csv(os.path.join(OUTPUT_DIR, "wp01.data.governance.csv"), sep="\t")
