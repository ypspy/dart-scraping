...
for file in path_list_out:
    ...
    if len(matrix) > 0:  # BS가 있을 때
        ...
        for i in matrix:
           for j in i:
               matches_pos = ["재고"]  # 매출/장기매출 ["매출채", "매출금"]
               matches_neg = ["충당"]  # 매출/장기매출 ["장기매출", "장기성매출", "충당"]

               j = "".join(j.split())

               if any(x in j for x in matches_pos) and all(x not in j for x in matches_neg):
...
df.to_csv(os.path.join(OUTPUT_DIR, "wp01.data15.output.csv"), sep="\t")
