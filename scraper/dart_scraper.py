...
def document_address_parser(i, startDate, endDate, reportType):
    ...
    while True:
        ...
        doc_key = '_'.join([reportType,
                            corpList[loop].a["href"][CORP_NO_SLICE],
                            docAddress[RCP_NO_SLICE],
                            reportName,
                            yearEnd
                            ])  # key 생성
...
def sub_document_address_parser(docAddress):
    """
    Parameters
    ----------
    docAddress : str
...
def get_html(htmlAddress, docKey):  # while loop으로 에러 부분 계속 시도
    while True:
        try:
            ...
            time.sleep(3)  # fixed: was time.delay(3)
            continue
...
def dart_scraper(reportType,  # report type 선택 A001: 사업보고서
    ...
    for i in tqdm(docsInThisPage, desc="docs in this page"):
        ...
        # 보고서 내부 개별 문서 주소 추출
        subDocAddressDict = sub_document_address_parser(docAddress)
